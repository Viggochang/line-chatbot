...
def my_registration():
    if request.method == "GET":
        ...
        past_registration_data = list(set([set(data[0:3]) for data in past_registration_data]))
        
        now_registration_data = CallDatabase.get_data("registration_data", condition = condition_now, order = order, ASC = True, all_data = True)
        now_registration_data = list(set([set(data[0:3]) for data in now_registration_data]))
