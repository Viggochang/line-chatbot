...
list_type = TextSendMessage(
    text = "請選擇查詢[已結束活動紀錄]或[正在進行活動紀錄]",
    quick_reply = QuickReply(
        items = [
            QuickReplyButton(
                action = PostbackAction(label = "已結束活動紀錄", data = "glist_已結束", display_text = "查詢[已結束活動紀錄]")
                ),
            QuickReplyButton(
                action = PostbackAction(label = "正在進行活動紀錄", data = "glist_進行中", display_text = "查詢[正在進行活動紀錄]")
                )
            ]))
